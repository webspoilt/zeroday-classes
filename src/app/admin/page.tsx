...
import {
    ...
    Target, BookOpen, Eye, ArrowLeft, ChevronRight, Lock, Unlock
} from 'lucide-react';
    ...
    return (
        <motion.form initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit}
            ...
            <div className="flex gap-3 pt-2">
                <button type="submit" className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-500 transition-all">
...
const QuestionForm = ({ initial, onSave, onCancel }: QFormProps) => {
    ...
    return (
        <motion.form initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit}
                ...
                {options.map((opt, i) => (
                    <div key={i} className="flex items-center gap-3">
                        <input
                            ...
                            className="w-4 h-4 text-green-500 border-slate-600 bg-slate-800"
                        />
            ...
            <div className="flex gap-3 pt-2">
                <button type="submit" className="flex items-center gap-2 px-5 py-2 bg-green-600 text-white rounded-lg font-bold hover:bg-green-500 text-sm">
                    <Save className="w-4 h-4" /> {initial ? 'Update Question' : 'Add Question'}
                </button>
...
function MockTestManager({ showToast }: { showToast: (msg: string) => void }) {
    ...
    const refresh = () => setTests(getMockTests());
    useEffect(() => { refresh(); }, []);
            ...
            <div>
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p className="text-slate-500 mt-1">Create tests, add questions with solutions — no coding needed.</p>
                    </div>
                    ...
                    {showTestForm && (
                        <div className="mb-8">
                            <TestMetaForm
                                ...
                                onSave={(data) => {
                                    if (editingTest) {
                                        updateMockTest(editingTest.id, data);
                                        showToast('✅ Test updated!');
                                    } else {
                                        addMockTest({ ...data, questions: [], isLocked: false, title: data.title || '', type: data.type || 'full', timeLimit: data.timeLimit || 120, negativeMarking: data.negativeMarking || 0 } as Omit<MockTest, 'id' | 'createdAt'>);
                            ...
                            {tests.map(test => (
                                <tr key={test.id} className="hover:bg-slate-800/30 transition-colors">
                                    ...
                                    <td className="px-5 py-4 text-center">
                                        <span className={`text-xs font-bold px-2 py-1 rounded ${test.type === 'full' ? 'bg-green-500/10 text-green-400' :
                                            test.type === 'subject' ? 'bg-blue-500/10 text-blue-400' :
                                                'bg-yellow-500/10 text-yellow-400'
                                            }`}>{test.type.toUpperCase()}</span>
                                    </td>
                                    ...
                                    <td className="px-5 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <button onClick={() => setSelectedTestId(test.id)}
                                                className="p-2 rounded-lg bg-slate-800 text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 transition-colors" title="Manage Questions">
                                                <Eye className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => { setEditingTest(test); setShowTestForm(true); }}
                                                className="p-2 rounded-lg bg-slate-800 text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 transition-colors" title="Edit">
                                                <Edit3 className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => { updateMockTest(test.id, { isLocked: !test.isLocked }); refresh(); showToast(test.isLocked ? '🔓 Test unlocked!' : '🔒 Test locked!'); }}
                                                className="p-2 rounded-lg bg-slate-800 text-slate-400 hover:text-yellow-400 hover:bg-yellow-500/10 transition-colors" title={test.isLocked ? 'Unlock' : 'Lock'}>
                                                {test.isLocked ? <Unlock className="w-4 h-4" /> : <Lock className="w-4 h-4" />}
                                            </button>
                                            {deleteConfirm === test.id ? (
                                                <div className="flex items-center gap-1">
                                                    <button onClick={() => { deleteMockTest(test.id); refresh(); setDeleteConfirm(null); showToast('🗑️ Test deleted.'); }}
                                                        className="p-2 rounded-lg bg-red-500 text-white hover:bg-red-400 text-xs font-bold">Confirm</button>
                                                ...
                                                <button onClick={() => setDeleteConfirm(test.id)}
                                                    className="p-2 rounded-lg bg-slate-800 text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-colors" title="Delete">
                        ...
                        {(showQForm || editingQ) && (
                            <div className="mb-6">
                                <QuestionForm
                                    ...
                                    onSave={(data) => {
                                        if (editingQ) {
                                            updateQuestionInTest(selectedTest.id, editingQ.id, data);
                                            showToast('✅ Question updated!');
                                        } else {
                                            addQuestionToTest(selectedTest.id, data);
                            ...
                            <div key={q.id} className="bg-slate-900/50 border border-white/5 rounded-xl p-4 hover:bg-slate-800/30 transition-colors">
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {q.explanation && (
                                            <div className="text-xs text-slate-500 mt-2 bg-white/5 rounded-lg px-3 py-1.5">💡 {q.explanation}</div>
                                        )}
                                    </div>
                                    <div className="flex items-center gap-1 shrink-0">
                                        <button onClick={() => { setEditingQ(q); setShowQForm(false); }}
                                            className="p-2 rounded-lg bg-slate-800 text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 transition-colors">
                                            <Edit3 className="w-3.5 h-3.5" />
                                        </button>
                                        {deleteQConfirm === q.id ? (
                                            <div className="flex items-center gap-1">
                                                <button onClick={() => { deleteQuestionFromTest(selectedTest.id, q.id); refresh(); setDeleteQConfirm(null); showToast('🗑️ Question deleted.'); }}
                                                    className="p-1.5 rounded-lg bg-red-500 text-white text-xs font-bold">Yes</button>
                                            ...
                                            <button onClick={() => setDeleteQConfirm(q.id)}
...
const TestMetaForm = ({ initial, onSave, onCancel }: TestMetaFormProps) => {
    ...
    return (
        <motion.form initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex gap-3 pt-2">
                <button type="submit" className="flex items-center gap-2 px-6 py-2.5 bg-green-600 text-white rounded-lg font-bold hover:bg-green-500">
                    <Save className="w-4 h-4" /> {initial ? 'Update Test' : 'Create Test'}
                </button>
                <button type="button" onClick={onCancel} className="px-6 py-2.5 bg-slate-800 text-slate-300 rounded-lg font-medium hover:bg-slate-700">Cancel</button>
            </div>
        </motion.form>
            ...
            <aside className="w-64 bg-slate-900/80 border-r border-white/5 p-6 flex flex-col h-screen sticky top-0 shrink-0">
                ...
                <nav className="space-y-1 flex-1">
                    <button onClick={() => setActiveTab('jobs')}
                        className={`flex items-center gap-3 w-full px-4 py-2.5 rounded-lg font-medium text-sm transition-colors ${activeTab === 'jobs' ? 'bg-blue-600/10 text-blue-400 border border-blue-500/10' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
                        <Briefcase className="w-4 h-4" /> Job Posts
                    </button>
                    <button onClick={() => setActiveTab('tests')}
                        className={`flex items-center gap-3 w-full px-4 py-2.5 rounded-lg font-medium text-sm transition-colors ${activeTab === 'tests' ? 'bg-green-600/10 text-green-400 border border-green-500/10' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
                    ...
                    <>
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <button onClick={() => { setShowForm(true); setEditingJob(null); }}
                                className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-500 transition-all shadow-lg">
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    ...
                                    <tbody className="divide-y divide-white/5">
                                        {jobs.map(job => {
                                            ...
                                            return (
                                                <tr key={job.id} className="hover:bg-slate-800/30 transition-colors">
                                                    ...
                                                    <td className="px-5 py-4 text-right">
                                                        <div className="flex items-center justify-end gap-2">
                                                            <button onClick={() => { setEditingJob(job); setShowForm(false); }}
                                                                className="p-2 rounded-lg bg-slate-800 text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 transition-colors" title="Edit">
                                                                <Edit3 className="w-4 h-4" />
                                                            </button>
                                                            <a href={job.applyLink} target="_blank" rel="noreferrer"
                                                                className="p-2 rounded-lg bg-slate-800 text-slate-400 hover:text-green-400 hover:bg-green-500/10 transition-colors" title="Open link">
                                        ...
                                        {jobs.length === 0 && (
                                            <tr><td colSpan={5} className="px-5 py-12 text-center text-slate-500">No job posts yet. Click &quot;Add New Job&quot; to get started.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
